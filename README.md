...
- [ ] Crie um projeto novo usando **Expo**, com o template `blank (TypeScript)`.
- [ ] Crie uma pasta chamada `components` dentro do projeto.
- [ ] Dentro da pasta `components`, crie um componente funcional chamado `Login.tsx`.
  - 💡 Você pode usar o atalho `rnf` no VSCode se tiver a extensão ES7+ instalada.
- [ ] Importe o componente `Login.tsx` no componente principal (`App.tsx`) e renderize dentro da `View`.
- [ ] No `Login.tsx`, crie **dois `TextInput`**, cada um com seu `useState`.
...
# 3. Inicie o projeto no navegador ou emulador
npx expo start
